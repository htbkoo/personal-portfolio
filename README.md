...
### Major updates
1. [`v6.0.0`](https://github.com/htbkoo/personal-portfolio/releases/tag/v5.0.0) - migrated to `pnpm` instead of `yarn` for the package manager
...
##### The `pnpm start` and `pnpm build` does not work on Windows machine

###### TL;DR solution
Set the `pnpm` / `yarn` / `npm` `script-shell` to `bash` (recommended) or `powershell` by running:
1. For `npm`, configure the `script-shell` value at `.npmrc`
   1. Reference: [pnpm/pnpm#2931](https://github.com/pnpm/pnpm/issues/2931#issuecomment-709619889)
2. For `yarn`, `yarn config set script-shell bash`
